...
int main() {
	...
	for (i = 0; i < segCount; i = i + 6) {
		...
		count++;
		printf("%f,%f,%f,%d \n", vertices[i+3], vertices[i + 4],vertices[i + 5],count);
	...
	*/
	...
	while (!glfwWindowShouldClose(window))																					   //检测是否关闭窗口
	{
		...
		int vertexColorLocation = glGetUniformLocation(shaderProgram, "ourColor");     //查询ourColor统一的位置，查询不到会返回-1
